...
#include <iostream>
#include <thread>
#include <cstdlib> // дл€ std::memcpy

using namespace std::chrono;

// ¬ызывает инициализацию SDL2 только один раз за врем€ работы приложени€.
// (в многопоточной среде нам потребуетс€ std::call_once вместо static bool)
void CUtils::InitOnceSDL2()
{
	static bool didInit = false;
	if (!didInit)
	{
		// ¬ случае успеха возвращаетс€ 0.
		...
		// ¬ случае успеха возвращаетс€ переданный параметр flags.
...
void CUtils::FlipSurfaceVertically(SDL_Surface& surface)
{
	...
	// «еркально отражаем пиксели по оси Y,
	//  если число строк пикселей в изображении нечЄтное,
	//  центральна€ строка остаЄтс€ нетронутой.
